...
class FillReport extends Component {
  ...
  render() {
    ...
    } else if (today.format < status.report_start_date || today.format > status.report_end_date) {
      return(
        <div className={style.fill_notice_close}>
          <h3 className={style.fill_notice_h3}>本轮汇报时间为：{status.report_start_date} 至 {status.report_end_date}</h3>
        </div>
      );
    } else {
          ...
          <Row gutter={0}>
            <Col span={14}>
              <Form horizontal onSubmit={this.handleSubmit}>
                ...
                >
                  <RangePicker
                    value={[moment(status.work_start_date, 'YYYY-MM-DD'), moment(status.work_end_date, 'YYYY-MM-DD')]}
                    format={'YYYY-MM-DD'}
                    disabled
                  />
                </FormItem>
            ...
            <Col span={8}>
              <h3 className={style.fill_notice_h3}>注意事项：</h3>
              <p className={style.fill_notice_word}>１、本次汇报开启时间：{status.report_start_date} ~ {status.report_end_date}</p>
              <p className={style.fill_notice_word}>２、本次审核开启时间：{status.review_start_date} ~ {status.review_end_date}</p>
              <p className={style.fill_notice_word}>３、工作汇报不必提及值班、值日、开会等常规工作</p>
              <p className={style.fill_notice_word}>４、工作汇报中不要填写与工作室无关的工作，如组织班级活动等</p>
              <p className={style.fill_notice_word}>５、另外有任何想说的话请填写在`意见建议`中</p>
            </Col>
          </Row>
        </div>
      );
