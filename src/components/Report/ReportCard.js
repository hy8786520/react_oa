      ...
      return (
        <Card title={this.renderTitle(item)} style={cardStyle}>
          ...
          <div className={style.report_item_salary}>
            ...
            {
              (item.status == 'expired')
              ? <InputNumber min={0} max={400} disabled value={item.salary.review} />
              : <InputNumber onChange={this.handleReviewSalaryChange} min={0} max={400} defaultValue={item.salary.review} />
            }
          </div>
          {
            // 行政及以上角色有权限查看最终薪资
            ...
            : <div className={style.report_item_salary}>
                <span className={style.report_item_title_span}>Ta的最终薪资：</span>
                {
                  // 往期汇报无法修改薪资
                  (item.status == 'expired')
                  ? <InputNumber min={0} max={400} disabled value={item.salary.final} />
                  : <InputNumber onChange={this.handleFinalSalaryChange} min={0} max={400} defaultValue={item.salary.final} />
                }
              </div>
          }
        </Card>
